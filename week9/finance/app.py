...
def buy():
    ...
    # remain amout of money
    remain = cash - price * shares

    if remain < 0:
        return apology("SORRY,Insufficient Cash ,Failed Purchase.")
...
def quote():
    ...
    if not row:
        return apology("invalid symbol")
    return render_template("quoted.html", name = row["name"], price = usd(row["price"]), symbol = row["symbol"])
